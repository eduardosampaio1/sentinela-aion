"use client";

import { signIn } from "next-auth/react";
import { useState } from "react";
import Image from "next/image";

const providers = [
  {
    id: "google",
    label: "Continuar com Google",
    icon: (
      <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden>
        <path
          fill="#4285F4"
          d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        />
        <path
          fill="#34A853"
          d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        />
        <path
          fill="#FBBC05"
          d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        />
        <path
          fill="#EA4335"
          d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        />
      </svg>
    ),
    envKey: "AUTH_GOOGLE_ID",
  },
  {
    id: "microsoft-entra-id",
    label: "Continuar com Microsoft",
    icon: (
      <svg viewBox="0 0 23 23" className="h-5 w-5" aria-hidden>
        <path fill="#f3f3f3" d="M0 0h23v23H0z" />
        <path fill="#f35325" d="M1 1h10v10H1z" />
        <path fill="#81bc06" d="M12 1h10v10H12z" />
        <path fill="#05a6f0" d="M1 12h10v10H1z" />
        <path fill="#ffba08" d="M12 12h10v10H12z" />
      </svg>
    ),
    envKey: "AUTH_ENTRA_ID",
  },
];

export default function LoginPage() {
  const [loading, setLoading] = useState<string | null>(null);

  async function handleSignIn(providerId: string) {
    setLoading(providerId);
    await signIn(providerId, { callbackUrl: "/" });
  }

  return (
    <div className="flex min-h-screen items-center justify-center bg-[var(--color-bg)] px-4">
      <div className="w-full max-w-sm">
        {/* Logo Sentinela AION (lockup horizontal completo) */}
        <div className="mb-8 flex flex-col items-center gap-3">
          <Image src="/logo-full.svg" alt="Sentinela AION" width={220} height={124} priority />
          <p className="mt-1 text-sm text-[var(--color-text-muted)]">
            Acesse com sua conta corporativa
          </p>
        </div>

        {/* Botões de provider */}
        <div className="space-y-3">
          {providers.map((p) => (
            <button
              key={p.id}
              onClick={() => handleSignIn(p.id)}
              disabled={loading !== null}
              className="flex w-full items-center justify-center gap-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3 text-sm font-medium text-[var(--color-text)] transition-colors hover:bg-white/5 disabled:opacity-50"
            >
              {loading === p.id ? (
                <span className="h-5 w-5 animate-spin rounded-full border-2 border-[var(--color-primary)] border-t-transparent" />
              ) : (
                p.icon
              )}
              {loading === p.id ? "Redirecionando…" : p.label}
            </button>
          ))}
        </div>

        {/* Rodapé */}
        <p className="mt-8 text-center text-xs text-[var(--color-text-muted)]">
          Acesso restrito a colaboradores autorizados.
          <br />
          Seu administrador de TI configura as permissões.
        </p>
      </div>
    </div>
  );
}
